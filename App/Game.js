...
class Game {
  ...
  init() {
    ...
    this.ship.init();
    this.elements.modalButton.addEventListener('click', () => {
      this.newGame();
    });
  }
  ...
  makeNewEnemy() {
    ...
    const enemy = new Enemy(this.elements.container, levelNumber, this.speed);
    enemy.init();
    this.enemies.push(enemy);
  }

  newLive() {
    const live = new Live(this.elements.container, this.speed);
    live.init();
  ...
  checkCollision() {
    this.enemies.forEach((enemy, Enemyindex, enemies) => {
      ...
      if (
        parseInt(enemy.element.style.top, 10) >
        window.innerHeight - enemy.element.offsetHeight - 8
      ) {
        enemy.explode();
      ...
      this.ship.bullets.forEach((bullet, index, bullets) => {
        ...
        if (parseInt(bullet.element.style.top, 10) + bullet.element.offsetHeight < 0) {
          bullet.remove();
    ...
    this.livesArr.forEach((live, liveIndex, lives) => {
      ...
      if (
        livePosition.bottom >= shipPosition.top &&
        livePosition.top <= shipPosition.bottom &&
        livePosition.right >= shipPosition.left &&
        livePosition.left <= shipPosition.right
      ) {
        live.remove();
        lives.splice(liveIndex, 1);
        this.lives++;
